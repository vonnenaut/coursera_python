...
def targeted_order(ugraph):
    ...
    while len(new_graph) > 0:
        ...
        for neighbor in neighbors:
        	if max_degree_node in new_graph[neighbor] and neighbor in new_graph:
...
def fast_targeted_order(ugraph):
	...
	for index in range(len(degree_sets)-1, -1, -1):
		...
		# When it encounters a non-empty set, the nodes in this set must be of maximum degree. The method then repeatedly chooses a node from this set, deletes that node from the graph, and updates degree_sets appropriately.
			...
			if arbitrary_node in new_graph[index]:
				...
				for neighbor in new_graph[arbitrary_node]:
					...
					if arbitrary_node in new_graph[neighbor]:
...
def run_suite():
    ...
    def analyze_methods():
 		""" comparatively analyzes run times of targeted_order and fast_targeted_order methods """
 		# Big-O bounds:
 		# targeted_order:
 		# n^2 + 5n
 		# fast_targeted_order:
		...
		for graph in upa_graphs:
			...
			elapsed_time_fto = time.time() - start_time
			fto_times.append(elapsed_time_fto)

		print "-------------------------------------------"
		total_to = 0
		for time in elapsed_time_to:
			total_to += time
		print "targeted_order elapsed_time:", total_to
		total_fto = 0
		for time in elapsed_time_fto:
			total_fto += time
		print "fast_targeted_order elapsed_time:", total_fto
