...
def fast_closest_pair(cluster_list):
    ...
    cl_copy = cluster_list[:]
    # print "\n\ncl_copy sorted (fast_closest_pair):", cl_copy, "\n\n"

    # n <-- length(P);
    len_cluster = len(cl_copy)

    # if n <= 3 then
    if len_cluster <= 3:
        # (d,i,j) <== SlowClosestPair(P);
        output_list = slow_closest_pair(cl_copy)
        # print "\n\n\n\n//////////// START slow_cp ////////////\n"
        ...
        left_output_list = fast_closest_pair(cl_copy[0:middle_index])
        right_output_list = fast_closest_pair(cl_copy[middle_index: len_cluster])
        ...
        mid = 1/2.0*(cl_copy[middle_index-1].horiz_center()+cl_copy[middle_index].horiz_center())
        ...
        half_width = abs(cl_copy[-1].horiz_center()-cl_copy[0].horiz_center())
        ...
        output_list = min(output_list, closest_pair_strip(cl_copy, mid, half_width))
        # print "\n\n///////////// END cp_strip //////////////\n\n\n\n"
    return output_list
...
def kmeans_clustering(cluster_list, num_clusters, num_iterations):
    ...
    for dummy_var in range(num_iterations):
        ...
        for index in range(num_clusters):
            cluster_groupings.append(alg_cluster.Cluster(set(), cluster_centers[index][0], cluster_centers[index][1], 0, 0))
        # # For each county
        # for j = 0 to n - 1 do
        for index in range(len_points_list):
            ...
            for idx, cluster in enumerate(cluster_groupings):
        ...
        # for f = 1 to k do
        for value in range(len(cluster_centers)):
            # muf = center (Cf)     // handled with Cluster class built-in method(s)
            # print "\n\n[cluster_groupings[value].horiz_center(), cluster_groupings[value].vert_center()]:", [cluster_groupings[value].horiz_center(), cluster_groupings[value].vert_center()]
            cluster_centers[value] = [cluster_groupings[value].horiz_center(), cluster_groupings[value].vert_center()]
