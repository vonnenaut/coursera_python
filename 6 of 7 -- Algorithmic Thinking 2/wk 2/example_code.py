...
def plot_distortions(distortions, data_table):
    ...
    plt.plot(xvals, yvals2, '-r', label='kmeans distortion')
    plt.xlabel('number of final clusters')
    plt.ylabel('distortion')
    plt.legend(loc='upper right')
    plt.title('# of Clusters vs Distortion using Hierarchical and K-means Algorithms')
    plt.show()
...
print "Data table loaded.  Creating clusters ..."
singleton_list = []
num_output_clusters = [6, 20]
plot_data = []
distortions = []

# compute distortions for range of output clusters (6 - 20) for both hierarchical and kmeans algorithms
for line in data_table:
    singleton_list.append(alg_cluster.Cluster(set([line[0]]), line[1], line[2], line[3], line[4]))
for value in range(num_output_clusters[0], num_output_clusters[1]+1):
    distortions.append(compute_distortion(singleton_list, data_table, value))

# create 3 plots, one for each data set (111, 290 and 896)
# plot #1:  111 for hierarchical and kmeans
# plot #2:  290
# plot #3:  896
plot_distortions(distortions, data_table)
