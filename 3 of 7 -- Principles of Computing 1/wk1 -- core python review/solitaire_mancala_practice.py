...
class SolitaireMancala:
    ...
    def get_board(self):
    	""" returns list of values for the store and each house """
    	return self._board
    ...
    def __str__(self):
        ...
        for dummy_var in self.get_board():
    ...
    def get_num_seeds(self, house_num):
        ...
        """
        try:
        	return self.get_board()[house_num]
	    except LookupError:
        	print "LookupError: self.get_board()[house_num] key does not exist"
        	return None
    ...
    def is_game_won(self):
        ...
        for dummy_var in range(1, len(self.get_board())):
			sum_of_seeds += self.get_board()[dummy_var]
        if sum_of_seeds is 0:
        	return True
        else:
        	return False
    ...
    def is_legal_move(self, house_num):
        ...
        """
        # if the value of a house number is the same as its index in the list (self._board), then the seed in that positions constitutes a legal move
        if self.get_board()[house_num] is house_num and house_num != 0:
        	return True
        else:
        	return False
    ...
    def apply_move(self, house_num):
        ...
        for lower_house_num in range(house_num):
        	self.get_board()[lower_house_num] = self.get_num_seeds(lower_house_num) + 1
        if house_num != 0:
        	self.get_board()[house_num] = 0
    ...
    def plan_moves(self):
        ...
        legal_moves = []
        move = -1
        board_sans_store = self.get_board()[1:]
...
def test_mancala():
    ...
    print "\nTesting choose_move - Computed:", my_game.get_board(), "Expected:", [1, 3, 0, 4, 4]

    # Owl test failure troubleshooting
    my_game.set_board(config7)
    my_game.apply_move(my_game.choose_move())
    print "\nTesting choose_move - Computed:", my_game.get_board(),"Expected:", [10, 2, 3, 4]
