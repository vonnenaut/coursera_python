...
class Apocalypse(poc_grid.Grid):
        ...
        while boundary.__len__() > 0:
            ...
            for neighbor_cell in neighbors:
                ...
                if visited.is_empty(neighbor_cell[0],neighbor_cell[1]) and self.is_empty(neighbor_cell[0],neighbor_cell[1]):
                    ...
                    distance = min([self.manhattan_distance(entity[0],entity[1],current_cell[0],current_cell[1]) for entity in entity_list])
    ...
    def move_humans(self, zombie_distance_field):
        ...
        for human in self.humans():
            best_location = max([zombie_distance_field[neighbor[0]][neighbor[1]] for neighbor in self.eight_neighbors(human[0], human[1])])
    ...
    def move_zombies(self, human_distance_field):
        ...
        are allowed
        """
        best_location = min([zombie_distance_field[neighbor[0], neighbor[1]] for neighbor in self.four_neighbors(zombie[0], zombie[1])])
...
def run_suite(format_function):
    ...
    def test_num_zombies():
        print "\nTesting num_zombies:"
        suite.run_test(apoc.num_zombies(), 1, "Test #1:")
    ...
    def test_num_humans():
        print "\nTesting num_humans:"
        suite.run_test(apoc.num_humans(), 1, "Test #3:")

    def test_add_human():
        print "\nTesting add_human:"
        apoc.add_human(3, 2)
        suite.run_test(apoc.num_humans(), 2, "Test #4:")

    def test_zombies():
        print "\nTesting zombies (generator):"
        for zombie in apoc.zombies():
            print zombie

    def test_humans():
        print "\nTesting humans (generator):"
        for human in apoc.humans():
            print human
    ...
    def test_manhattan_distance():
        ...
        neighbors = [(1,1)]
        for neighbor_cell in neighbors:
            print neighbor_cell, ":", apoc.manhattan_distance(neighbor_cell[0], neighbor_cell[1], current_cell[0], current_cell[1])
    ...
    def test_move_humans():
        print "\nTesting move_humans:"
        dist_field = apoc.compute_distance_field(ZOMBIE)
        apoc.move_humans(dist_field)

    def test_move_zombies():
        print "\nTesting move_zombies:"
        dist_field = apoc.compute_distance_field(HUMAN)
        apoc.move_zombies(dist_field)
    ...
    if format_function == "all":
        ...
        test_move_humans()
        test_move_zombies()
    elif format_function == "num_zombies":
        test_num_zombies()
    elif format_function == "add_zombie":
        test_add_zombie()
    elif format_function == "num_humans":
        test_num_humans()
    elif format_function == "add_human":
        test_add_human()
    elif format_function == "zombies":
        test_zombies()
    elif format_function == "humans":
        test_humans()
    elif format_function == "manhattan_distance":
        test_manhattan_distance()
    elif format_function == "compute_distance_field":
        test_compute_dist_field()
    elif format_function == "move_humans":
        test_move_humans()
    elif format_function == "move_zombies":
        test_move_zombies()
